...
- **v2.2** — ultraplan pass (8 parallel reviewers — 4 plan-focused R1-R4, 4 agent-focused A1-A4). Major changes:
  - **Schema contract gaps fixed** (R1): added `priority` and `anchor_event_id` to composition.RenderedCaption; added `dropped_captions` and `style.preset` to composition; added `artifact_sha256` to manifest stages.
  - **Cascading invalidation** now content-aware (D17) — re-runs producing identical hashes skip downstream cascade. Directory artifacts (sfx) invalidate by recursive sibling clear.
  - **Schema lock softened** (D2 update): additive non-breaking changes allowed PM-signoff in DECISIONS.md; breaking changes require schema_version bump per D14.
  - **D12-D19 added**: ducking via ffmpeg pre-mix; OffthreadVideo locked for renderMedia; schema-bump policy; holdout rotation (90 days); eval cost cap revised to $25/phase-eval and $15/agent-refresh; content-aware invalidation; contracts.json registry; judge-version stamping on baselines.
  - **Phase 1 expanded**: stub composition-director must produce schema-valid composition.json round-trippable through Pydantic AND Zod; `dvg render` uses `bundle()` + `selectComposition()` + `renderMedia` chain with bundle cached at `runs/<ts>/.remotion-bundle/`; `--keep-runs N=20` default ships from Phase 1 (was Phase 9 polish).
  - **Phase 2 entry**: D11 spike now requires per-option evidence bundles (5s sample MP4 + verified Playwright launch + DOM events still firing + clean ffmpeg region). avfoundation device enumeration enters `dvg doctor`.
  - **Phase 3**: PySceneDetect concrete recall/precision exit criterion (≥80% recall, ≤30% FP on PM-labeled fixtures); visual-analyst keyframe count cap of 8/min footage (cost guard).
  - **Phase 4**: Lyria smoke must do 5-prompt test (not single-call); cost-cap revisit at Phase 4 entry firmer; stitch-tempo-continuity check added; vocals-leak detection in eval.
  - **Phase 5**: Kenney pack curated DOWN to 30-50 clips (from ~150); explicit "Linear not Mario" aesthetic anchor.
  - **Phase 6**: caption-timing resolver failure spec for unresolvable anchors; ebur128 assertion shipped as pytest helper (reused by Phase 8); per-fixture caption-mask polygons for perceptual diff.
  - **Phase 8**: severity ladder defined (high/medium/low); proposed_action enum {regenerate_stage, escalate, accept_with_warning} with hard-allowlist of auto-retryable codes; tool versions pinned in `dvg doctor`; canonical-scalar snapshot tests (not raw stderr).
  - **Phase 11**: success criteria add length bounds (20-180s), captions density, regression suite green at tag, two-run determinism check; severity ladder must be locked; `dvg doctor` clean-machine check for PyPI gate.
  - **Agent designs committed** (A1-A4): full per-agent design captured in `.claude/agents/<name>/design.md` for all 9 agents. New `_shared/` files: `refresh-protocol.md`, `eval-rubric-skeleton.md`, `section-loader.md`. `refresh.md` shape includes "Pin facts" for targeted verification. `inspiration.md → core.md` graduation rule defined.
